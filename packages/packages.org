...
#+begin_src bash
  function spinner() {
  	...
  	gum spin --show-output --spinner points --title "$@"
  }
#+end_src
...
* kc
:PROPERTIES:
:header-args:bash: :tangle kc :shebang #!/usr/bin/env bash :comments link
:END:
** script
*** Init
#+begin_src bash :noweb yes
...
#+begin_src bash
  spinner "Logging into $CLUSTER" -- \
  	az aks get-credentials --only-show-errors --name "$CLUSTER" --resource-group "$RESOURCE_GROUP" --subscription "$SUBSCRIPTION" || exit 1
  kubelogin convert-kubeconfig -l azurecli
#+end_src

** nix derivation
#+begin_src nix :tangle kc.nix :comments link
  { pkgs, ... }:
  {
  ...
  for repo in "${REPOS[@]}"; do
  	...
  	if [ -d "$path" ]; then
  		...
  		spinner "Downloading $PROJECT/$repo" -- \
  			git clone "$url" "$path" --bare --origin origin || true
  	fi
  done
#+end_src
