...
#+begin_src emacs-lisp :tangle yes
  (defvar cloveynit-magit-feature-base-directory
    (expand-file-name "feature" "~/")
    "Default directory for git worktree feature branches")

  (defun cloveynit-magit-feature-worktree-branch (work-item title repo-name)
    (interactive
     `(,(read-string "Work item reference: ")
       ,(read-string "Work item title: ")
       ,(read-string "Repository name: " (project-name (project-current)))))
    (let ((path (expand-file-name (s-concat work-item "-" (denote-sluggify-title title) "__" repo-name) cloveynit-magit-feature-base-directory))
  	(branch (s-concat "feature/" work-item "-" (denote-sluggify-title title)))
  	(start-point (magit-read-starting-point "Starting branch: ")))
...
#+begin_src emacs-lisp :tangle yes
  (use-package url-http
    :defer t
    :config
    (defun cloveynit-url-http-create-request-host-header-fix (input-request)
      "Advice function for 'url-http-create-request' that will modify the return value to provide only the last Host header. This should allow for overriding the Host header."
      (let ((lines (split-string input-request "\r\n"))
            (host-header nil)
            (request '()))
        (dolist (line lines)
          (cond
           ((string-prefix-p "Host: " line)
            (setq host-header line))
           ((and (string-empty-p line) host-header)
  	  (push host-header request)
  	  (setq host-header nil)
            (push line request))
           (t (push line request))))
        (mapconcat 'identity (nreverse request) "\r\n")))

    (advice-add
     #'url-http-create-request
     :filter-return #'cloveynit-url-http-create-request-host-header-fix))
#+end_src
