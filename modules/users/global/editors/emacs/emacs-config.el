...
(use-package emacs :config (setopt select-active-regions nil))
...
(defvar cnit/languages--default-formatters-alist
  '(("Nix" nixfmt) ("YAML" yamlfmt) ("Emacs Lisp" elisp-autofmt)))
...
(defvar cnit/major-mode--default-file-extenson
  '((emacs-lisp-mode . ".el") (bash-ts-mode . ".sh")))
...
(transient-define-prefix
 cnit/modes-dispatch () "Transient for toggling minor modes."
 :transient-suffix 'transient--do-stay
 [["Modes" ("fm" (lambda ()
      (cnit/modes-highlight 'flymake-mode "Flymake"))
    flymake-mode)
   ("fc" (lambda ()
   ...
   ("fa" (lambda ()
      (cnit/modes-highlight 'format-all-mode "Format all"))
    format-all-mode)
   ("hl" (lambda ()
      (cnit/modes-highlight 'hl-line-mode "Highlight Line"))
    hl-line-mode)
   ("ln" (lambda ()
      (cnit/modes-highlight
   ...
   ("ww" (lambda ()
      (cnit/modes-highlight 'word-wrap-whitespace-mode "Word Wrap"))
    word-wrap-whitespace-mode)
   ("cn" (lambda ()
      (cnit/modes-highlight 'column-number-mode "Column Number"))
    column-number-mode)
   ("ar" (lambda ()
      (cnit/modes-highlight 'auto-revert-mode "Auto Revert"))
    auto-revert-mode)
   ("fs" (lambda ()
      (cnit/modes-highlight 'flyspell-mode "Flyspell"))
    flyspell-mode)
   ("ps" (lambda ()
      (cnit/modes-highlight
  ...
  ["Indent" ("ei" (lambda ()
   ...
   ("it" (lambda ()
      (cnit/modes-highlight 'indent-tabs-mode "Indent tabs"))
    indent-tabs-mode)
   ("ib" (lambda ()
      (cnit/modes-highlight 'indent-bars-mode "Indent bars"))
    indent-bars-mode)]
  ["Whitespace" ("wb" (lambda ()
      (cnit/modes-highlight 'ws-butler-mode "WS Butler"))
    ws-butler-mode)
   ("ws" (lambda ()
      (cnit/modes-highlight 'whitespace-mode "Whitespace"))
    whitespace-mode)]
  ["Parens" ("rb" (lambda ()
      (cnit/modes-highlight 'rainbow-mode "Rainbow"))
    rainbow-mode)
   ("ep" (lambda ()
      (cnit/modes-highlight 'electric-pair-mode "Electric Pair"))
    electric-pair-mode)
   ("sp" (lambda ()
      (cnit/modes-highlight 'show-paren-mode "Show Paren"))
    show-paren-mode)]])
...
(use-package
 ...
 :config (setopt whitespace-style '(face tab-mark trailing)))
...
(use-package
 ...
 (setopt
  ...
  vertico-cycle t
  vertico-buffer-display-action '(display-buffer-in-side-window (side . bottom))))
...
(use-package
 ...
 :config
 (setopt vertico-multiform-commands '((consult-line buffer)))
 (setopt vertico-multiform-categories '((consult-grep buffer))))
...
(use-package
 ...
 :bind (("C-c c" . cnit/consult-dispatch))
 :init
 (setopt
...
(transient-define-prefix
 cnit/consult-dispatch () "Transient for Consult commands."
 [["Buffers"
...
(use-package
 ...
 (-each
  cnit/languages--org-src-lang-modes
...
(transient-define-prefix
 cnit/denote-dispatch () "Transient for Denote commands."
 [["Notes"
   ("n" "New" denote)
 ...
 [["File" ("r" "Rename" denote-rename-file)
   ("R"
    "Rename from front matter"
    denote-rename-file-using-front-matter)]
  ["Folder" ("s" "Search" cnit/find-file-in-notes)
   ("p" "Dired"
    (lambda ()
      (interactive)
          ...
          (file
           (completing-read
            ...
            (project--files-in-directory
             denote-directory vc-dirs-ignores))))
     (when file
       (find-file file)))))
...
(use-package
 yasnippet
 :config
 (setq-default yas-keymap-disable-hook
               (lambda ()
...
(use-package
    ...
    (defmacro cnit/transient-define-suffix-mode (title mode)
        `(transient-define-suffix
             ,(intern (format "cnit/%s-suffix" mode))
             ()
             ,(format "Toggle %s mode." mode)
             :description
             (lambda ()
                 (cnit/modes-highlight
                     ',(intern (format "%s-mode" mode)) ,title))
             (interactive)
             (,(intern (format "%s-mode" mode))
                 (if (bound-and-true-p ,(intern (format "%s-mode" mode)))
                     0
                     1))))

    (cnit/transient-define-suffix-mode "Fold" treesit-fold)
    (cnit/transient-define-suffix-mode
        "Indicators" treesit-fold-indicators)
    ...
    (transient-define-prefix
        ...
        [["Actions"
             ...
             ("C"
                 ...
                 :inapt-if-nil treesit-fold-mode)
             ("O"
                 "Open All"
             ...
             ("t"
                 ...
                 treesit-fold-toggle
                 :inapt-if-nil treesit-fold-mode)]
            ["Settings" ("l" "Line Count"
                            (lambda ()
                                (interactive)
                                (setopt treesit-fold-line-count-show
                                    (if treesit-fold-line-count-show
                                        nil
                                        t))
                                :inapt-if-nil treesit-fold-mode))]
            ["Modes"
    ...
    :config (setopt treesit-fold-line-count-show t)
    :bind ("C-c f" . cnit/treesit-fold))

(use-package ws-butler :init (ws-butler-global-mode t))

(use-package
 elec-pair
 :hook (after-init . electric-pair-mode)
 :config (setopt electric-pair-open-newline-between-pairs t))
   ...
   (save-excursion
     (avy-with
      ...
      (let ((avy-all-windows nil))
        (when-let* ((char2 (read-char "char: "))
                    (pt2
                     (cdr
                      (avy-process
                       (avy--regex-candidates
                        (regexp-quote (string char2)) pt)))))
...
(use-package
 ...
 (defvar cnit/last-gptel-project-root nil)
                   ...
                   (lambda (arg)
                     (and (stringp arg)
                          (string-match-p
                           ...
                           arg)))
                   args))
         ;; Only restart if server is running (optional)
         (when (functionp 'mcp-stop-server)
           (mcp-stop-server server-name))
         (when (functionp 'mcp-hub-start-server)
           (mcp-hub--start-server server))))))

 (defun cnit/restart-servers-if-project-changed (&rest _)
   "Restart MCP servers if the current project root has changed."
   (let ((current-root
          (or (and (project-current) (project-root (project-current)))
              "~/")))
     (unless (equal current-root cnit/last-gptel-project-root)
       ;; Put your server-restarting logic here
       (mcp-restart-servers-for-project current-root)
       (setq cnit/last-gptel-project-root current-root))))

 (advice-add
  'gptel-send
  :before #'cnit/restart-servers-if-project-changed)
 (advice-add
  'gptel-menu
  :before #'cnit/restart-servers-if-project-changed)

 (defun cnit/restart-servers-on-gptel-window-select (&rest _)
   "Restart MCP servers if the gptel window is selected."
   (when (bound-and-true-p gptel-mode)
     (cnit/restart-servers-if-project-changed)))

 (setq window-selection-change-functions
       (cons
        #'cnit/restart-servers-on-gptel-window-select
        window-selection-change-functions))

 ;; Start the MCP hub servers with a default path
 (cnit/set-mcp-hub-servers-path
  (or (and (project-current) (project-root (project-current))) "~/"))
...
(use-package
 ...
 (defun cnit/reorder-eldoc-functions ()
   ...
   (setq eldoc-documentation-functions
         (cons
          ...
          (remove
           #'flymake-eldoc-function eldoc-documentation-functions))))
 ...
 (add-to-list
  ...
  `(nix-ts-mode . ,(cdr (assoc 'nix-mode eglot-server-programs))))
...
(use-package zig-mode :mode ("\\.zig\\'" . zig-mode))
...
(use-package
 ...
 :mode "\\.ya?ml\\'"
 :hook
 ((yaml-ts-mode . cnit/yaml-ts-mode-hook)
...
(use-package
 ...
 (setopt language-id--definitions
         (append '(("Nix" nix-ts-mode)) language-id--definitions)))
...
If the file does not exist, return an empty list."
  (let ((file-path
         (expand-file-name ".pre-commit-config.yaml" directory)))
    (if (not (file-exists-p file-path))
      ...
      (let* ((yaml-string
             ...
             (ids '("all")))
        (cl-loop
         for repo across (gethash 'repos pre-commit-config) do
         (cl-loop
          for
          hook
          across
...
TARGET is the pre-commit id to run."
  (when-let* ((executable
               (cnit/transient-compile--tool-property
                'pre-commit
                :exe))
              (target
               (if (string= "all" target)
                   ""
                 target)))
    (transient-compile--shell-join
     executable "run" target "--all-files")))

(defun cnit/compile--nix-command (args)
  "Run nix with ARGS in DIRECTORY."
  (when-let* ((executable
               (or (cnit/transient-compile--tool-property 'nix :exe)
                   "nix"))
              (nix-bin (executable-find executable))
              (shell-result
               (shell-command (string-join `(,nix-bin ,args) " ")
                              "*nix-command*"
                              "*nix-error*")))
    (if (eq shell-result 0)
        (with-current-buffer "*nix-command*"
          (buffer-string))
      nil)))
...
(defun cnit/compile--nix-flake-targets (directory)
  ...
  (when-let* ((json-raw
               (cnit/compile--nix-command
                (string-join `("flake show" ,directory "--json")
                             " ")))
              (json-data (json-parse-string json-raw))
              (nixos-configs
               (gethash "nixosConfigurations" json-data)))
    (-map
     (lambda (config-name) (string-join `("os " ,config-name)))
     (hash-table-keys nixos-configs))))
    ...
    (dolist (tool tools)
      (let ((match (transient-compile--tool-property tool :match)))
        (cond
         ...
         (t
          (setq combined-matches
                (append combined-matches (list match)))))))
           ...
           (lambda (tool)
             `(when-let* ((transient-compile-tool ',tool)
                ...
                (if (string= detected-dir directory)
                    (-map
                     (lambda (target)
                       (format "[%s] %s" ',tool target))
                     (transient-compile--tool-targets
                      transient-compile-tool directory)))))
           tools))))))
...
into one transient menu."
  ...
  (let ((combined-name (intern (mapconcat 'symbol-name tools " & ")))
        (target
         (intern
          (apply #'cnit/compile--combine-function-name tools))))
    `(,combined-name
      :match
      ,(apply #'cnit/compile--combine-tools-matchers tools)
      :chdir t
      :targets ,target
...
arguments."
  (when (string-match "\\[\\([^]]*\\)\\] \\(.*\\)" target)
    (let* ((tool (intern (match-string 1 target)))
           (target (match-string 2 target))
           (compile-command
            (transient-compile--tool-property tool :command)))
...
(use-package
 ...
 (add-to-list
  'transient-compile-tool-alist
  (cnit/compile--combine-tools 'nix 'pre-commit 'make)))
...
(use-package
 ...
 (defun cnit/org-src-editorconfig ()
   (when-let* ((ext
                (alist-get
                 major-mode cnit/major-mode--default-file-extenson))
               (buffer-file-name
                (concat
                 default-directory (make-temp-name "") (or ext ""))))
...
(use-package dired :config (setopt dired-dwim-target t))

(use-package dired-x :hook (dired-mode . dired-omit-mode))
...
(use-package
 ...
 (defvar cnit/dim-overlays nil
   "List of overlays used to dim all windows.")

 (defun cnit/dim-all-windows-foreground (&optional color)
   "Overlay all visible windows with foreground COLOR."
   (setq cnit/dim-overlays
         (mapcar
          (lambda (win)
            (with-current-buffer (window-buffer win)
              (let ((ov
                ...
                (overlay-put
                 ov 'face `(:foreground ,(or color "gray60")))
                ov)))
          (window-list))))

 (defun cnit/clear-dim-all-windows ()
   "Clear overlays used to dim all windows."
   (mapc #'delete-overlay cnit/dim-overlays)
   (setq cnit/dim-overlays nil))
 :init
 (setopt display-buffer-base-action
         '((display-buffer--maybe-same-window
            display-buffer-reuse-window display-buffer-ace-window)))
 (advice-add
  'corfu-popupinfo--show
  :around #'safe-corfu-popupinfo--show)
 ...
 (defun display-buffer-ace-window (buffer alist)
   (let* ((aw-dispatch-always t)
          ...
          (aw-dispatch-alist
           (append
            '((?q
               (lambda (_w)
                 (selected-window))
               "quit and return current window"))
            aw-dispatch-alist))
     ...
     (if (= num-windows 1)
         (unwind-protect
             (progn
               (cnit/dim-all-windows-foreground "gray60")
               (message "Switching to %s" buffer)
               (setq window (aw-select nil)))
           (cnit/clear-dim-all-windows))
       (progn
         (message "Switching to: %s" buffer)
         (setq window (aw-select nil))))
...
(defun cnit/read-ticket-number ()
  (completing-read
   "Select ticket number: " (cnit/get-ticket-numbers)))

(defun cnit/get-ticket-name (ticket-number)
  (let* ((feature-dir (expand-file-name "~/feature/"))
         (folders
          (directory-files feature-dir
                           nil
                           (format "^%s-.*" ticket-number)))
         (existing-names
          (mapcar
           (lambda (dir)
             (let* ((name (file-name-nondirectory dir))
    ...
    (if existing-names
        (completing-read
         "Select ticket name: " (delete-dups existing-names))
...
(defun cnit/magit-repo-name ()
  (when-let* ((repo-name (magit-get "remote" "origin" "url"))
              (git-removed
               (replace-regexp-in-string
                "\\.git$" "" (file-name-nondirectory repo-name))))
...
(defun cnit/magit-worktree-names ()
  (let* ((ticket (cnit/read-ticket-number))
         (kebab-name
          (denote-sluggify-title (cnit/get-ticket-name ticket))))
    (cnit/magit-worktree-names-format
     (format "%s-%s" ticket kebab-name)
     (cnit/magit-repo-name-formatted))))
...
(eval-after-load 'magit
  (progn
    ...
    (transient-append-suffix
     'magit-worktree "c"
     '("f" "Feature worktree" cnit/magit-worktree-new))
    `(transient-append-suffix
      'magit-worktree "c"
      '("w" "Feature checkout" cnit/magit-worktree-checkout))))
...
Selection is by organisation under the git-clones root directory"
  ...
  (let* ((root (expand-file-name "~/git-clones"))
         (org
          (completing-read
           "Select organisation: "
           (-map (lambda (f) (f-filename f)) (f-directories root))))
