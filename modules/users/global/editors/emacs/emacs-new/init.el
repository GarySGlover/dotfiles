...
(with-eval-after-load (require 'meow)
  (defvar-keymap cnit-meow-tools-prefix-map
    :doc "Tools prefix"
    :prefix
    'cnit-meow-tools-prefix
    "g"
    #'gptel-menu)
  (defvar-keymap cnit-meow-mark-prefix-map
    :doc "Meow mark map"
    :prefix
    'cnit-meow-mark-prefix
    "g"
    #'meow-grab
    "d"
    #'meow-mark-defun
    ...
    #'meow-mark-uuid
    "h"
    #'meow-mark-whitespace
    "a"
    #'meow-beginning-of-thing
    "e"
    #'meow-end-of-thing)
  (defvar-keymap cnit-meow-next-prefix-map
    :doc "Meow next map"
    :prefix
    'cnit-meow-next-prefix
    "w"
    #'meow-next-word
    "s"
    #'meow-next-symbol)
  ...
  (define-key
   ...
   [remap cnit-meow-insert]
   'meow-beacon-insert)
  (define-key
   meow-beacon-state-keymap
   [remap cnit-meow-append]
   'meow-beacon-append)
  (define-key
   meow-beacon-state-keymap
   [remap cnit-meow-delete]
   'meow-beacon-noop)
  (meow-global-mode 1))

(defun cnit-meow-left ()
  (interactive)
  (if (region-active-p)
      (meow-left-expand)
    (meow-left)))

(defun cnit-meow-right ()
  (interactive)
  (if (region-active-p)
      (meow-right-expand)
    (meow-right)))

(defun cnit-meow-prev (arg)
  (interactive "P")
  (if (region-active-p)
      (meow-prev-expand arg)
    (meow-prev arg)))

(defun cnit-meow-next (arg)
  (interactive "P")
  (if (region-active-p)
      (meow-next-expand arg)
    (meow-next arg)))

(defun cnit-meow-delete ()
  "Deletes region if active otherwise calls meow-delete."
  (interactive)
  (if (region-active-p)
...
(defun cnit-meow-mark (arg)
  (interactive "P")
  (if (meow--with-universal-argument-p arg)
      (let ((current-prefix-arg nil))
        (set-transient-map cnit-meow-mark-prefix))
