...
Will need to run the following to install some depedencies

#+begin_src bash
  sudo apt install physlock nscnd
#+end_src
...
https://gofranz.com/blog/screen-sharing-on-niri-wayland-with-guix/

#+begin_src nix-ts :tangle modules/non-nix.nix :noweb yes
  {
    flake.aspects = {
      nonNix.homeManager =
        { pkgs, ... }:
...
So far this doesn't fix the screen sharing
#+begin_src bash
  dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP=niri

  while ! busctl --user status org.gnome.Mutter.ScreenCast >/dev/null 2>&1; do sleep 0.2; done;
#+end_src

Need to fix the filesystem space
#+begin_src bash
