...
# Create a temporary GIT_ASKPASS file
GIT_ASKPASS=$(mktemp)
...
function get_azure_token() {
	...
	AZURE_TOKEN=$(az account get-access-token --resource 499b84ac-1321-427f-aa17-267ca6975798 -o json) || {
...
# Function to generate a PAT token
function generate_pat_token() {
	local scope="vso.code"
	local display_name
	display_name="Backup Script $(date +%Y-%m-%dT%H:%M:%SZ)"
	local expiration_days=1

	# Construct the correct PAT token endpoint
	local pat_url="https://vssps.dev.azure.com/$DEVOPS_ORG/_apis/tokens/pats?api-version=7.0-preview.1"

	# Use Azure DevOps REST API to create a PAT
	local pat_response
	pat_response=$(curl -s -X POST \
		-H "Authorization: Bearer $(get_azure_token)" \
		-H "Content-Type: application/json" \
		-d "{\"displayName\":\"$display_name\",\"scope\":\"$scope\",\"validTo\":\"$(date -u -d "+$expiration_days days" +%Y-%m-%dT%H:%M:%SZ)\"}" \
		"$pat_url") || {
		echo "Error: Failed to generate PAT token." >&2
		exit 1
	}

	echo "$pat_response" | jq -r '.patToken'
}

# Function to revoke the PAT token
function revoke_pat_token() {
	local authorization_id
	authorization_id=$(echo "$PAT_TOKEN" | jq -r '.authorizationId')

	if [[ -n $authorization_id ]]; then
		local revoke_url="https://vssps.dev.azure.com/$DEVOPS_ORG/_apis/tokens/pats?authorizationId=$authorization_id&api-version=7.1-preview.1"

		# Revoke the PAT token using Azure DevOps REST API
		curl -s -X DELETE \
			-H "Authorization: Bearer $(get_azure_token)" \
			"$revoke_url" || {
			echo "Warning: Failed to revoke PAT token." >&2
		}
	fi
}

# Update the trap to revoke the PAT token and remove the GIT_ASKPASS file
trap 'revoke_pat_token; rm -f "$GIT_ASKPASS"' EXIT

# Generate a PAT token for Git operations
PAT_TOKEN=$(generate_pat_token)
readonly PAT_TOKEN

# Extract the actual PAT token from the JSON
PAT_TOKEN_VALUE=$(echo "$PAT_TOKEN" | jq -r '.token')

# Write the PAT token to the GIT_ASKPASS file
echo "echo \"${PAT_TOKEN_VALUE}\"" >"$GIT_ASKPASS"
chmod +x "$GIT_ASKPASS"
export GIT_ASKPASS
