...
Emacs always displaying the \ast{}Warnings\ast{} buffer on startup.
...
* Key Bindings
** COMMENT Transient auto builder
#+begin_src emacs-lisp :tangle yes
  (let ((grouped-targets
  ...
  (defface cnit/transient-builder-keychar
    '((t :inherit font-lock-string-face :underline t))
    "Face to highlight key character inside group or target name.")

  (defun cnit/transient-builder--propertize (word word-index)
    "Highlight key character at WORD-INDEX inside WORD."
    (add-text-properties
     word-index (1+ word-index) '(face cnit/transient-builder-keychar)
     word))

  (defvar cnit/transient-highlight-function nil
    "Function to apply conditional highighting to transient menu items.")


  (defface cnit/transient-modes-local-highlight '((t :inherit success))
    "")
  (defun cnit/transient-modes-local-highlight (mode-text)
    (when (and mode-name
               (symbol-value
                (intern (format "%s-mode" (s-dashed-words mode-text)))))
      (add-text-properties
       0 (length mode-text) '(face cnit/transient-modes-local-highlight)
       mode-text)))

  (cnit/transient-modes-local-highlight "flymake")
  (cnit/transient-modes-local-highlight "format all")

  (defun cnit/transient-builder ()
    (transient-define-prefix
     cnit/modes-dispatch-test ()
     :transient-suffix 'transient--do-stay
     [[:description
       (lambda ()
         (let ((w "default"))
           (cnit/transient-builder--propertize w 0)
           w))
       ("df" (lambda ()
          (let ((w "flymake"))
            (cnit/transient-modes-local-highlight w)
            (cnit/transient-builder--propertize w 0)
            w))
        flymake-mode)]
      [:description
       ...
       ("ep" (lambda ()
          (let ((w "electric pair"))
            (cnit/transient-modes-local-highlight w)
            (cnit/transient-builder--propertize w 9)
            w))
        electric-pair-mode)
       ("ei" (lambda ()
          (let ((w "electric indent"))
            (cnit/transient-modes-local-highlight w)
            (cnit/transient-builder--propertize w 9)
            w))
